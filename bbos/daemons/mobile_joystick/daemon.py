...
def run(w_joy, r_cam, rate, response_time=1, wait_time=5, port=8000):
    ...
    <title>Dual Joystick with Camera</title>
    <style>
    body {
      margin: 0;
      background: #111;
...
def main():
    ...
    rate = Time(CFG.rate)
    with Writer('/mobile.joystick', Type("joystick")) as w_joy, \
         Reader('/camera.jpeg') as r_cam :
        run(w_joy, r_cam, rate)
    print(rate.stats)
