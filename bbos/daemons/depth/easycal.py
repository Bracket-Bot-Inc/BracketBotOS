...
def main():
    ...
    parser.add_argument('--port', type=int, default=8082, help='Web server port (default: 8082)')
    parser.add_argument('--bootstrap', type=str, 
                       default='./stereo_calib.yaml',
                       help='Path to initial calibration YAML file (default: stereo_calib.yaml)')
    ...
    try:
        ...
        device_id = 0
        print(f"{GREEN}Using camera device: /dev/video{device_id}{RESET}")
        cap = cv2.VideoCapture(device_id)
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 2560)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
        
        if not cap.isOpened():
            print(f"{RED}Could not open camera device {device_id}{RESET}")
            sys.exit(1)
        
        print(f"{GREEN}Camera initialized successfully.{RESET}")
        
        # Load bootstrap calibration (default: stereo_calib.yaml)
        bootstrap_calib = None
        if os.path.exists(args.bootstrap):
            bootstrap_calib = load_calibration_yaml(args.bootstrap)
            if bootstrap_calib is not None:
                print(f"{GREEN}Bootstrap calibration loaded successfully from {args.bootstrap}{RESET}")
                print(f"{GREEN}Using existing calibration as starting point for faster convergence{RESET}")
            else:
                print(f"{YELLOW}Warning: Could not load bootstrap calibration from {args.bootstrap}{RESET}")
                print(f"{YELLOW}Continuing with standard calibration from scratch...{RESET}")
        else:
            print(f"{YELLOW}Warning: Bootstrap file {args.bootstrap} does not exist{RESET}")
            print(f"{YELLOW}Continuing with standard calibration from scratch...{RESET}")
        
        # Create calibrator
        calibrator = CameraCalibrator(port=args.port, bootstrap_calib=bootstrap_calib)
